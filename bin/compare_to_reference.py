...
def __get_average(size_list):
    if size_list:
        return str("%.0f" % (sum(size_list)/float(len(size_list))))
    return 0

def __get_last_sorted_value(size_list):
    if size_list:
        return str(sorted(size_list)[-1])
    return 0
...
def __get_table_data(c):
    ...
    tmp[0].append(__get_average(ref_gaps))
    tmp[1].append(__get_last_sorted_value(ref_gaps))
    tmp[0].append(__get_average(query_gaps))
    tmp[1].append(__get_last_sorted_value(query_gaps))
    table_data += tmp    
