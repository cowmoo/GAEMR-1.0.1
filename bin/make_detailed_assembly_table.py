...
def __get_genus_from_tax_list(tax_list):
    for j in tax_list:
        group,name = j.split('=')
        if name == 'NO_HIT'or group == 'genus':
            return name
    return "NA"
        ...
        for i in f.readlines():
            if re.match('^#', i):
                if re.search('TITLE', i):
                    ...
                    most_common_org = re.sub("\).*","",re.sub(".*\(","",i.rstrip()))
                    
            else:
                data = i.rstrip().split(constant.TABLE_DELIMITER)
                contig = data[0]
                taxonomy[contig] = {}
                taxonomy[contig]['hit len'] = int(re.sub(",","",data[2]))
                taxonomy[contig]['genus'] = __get_genus_from_tax_list(data[4].split(';'))
                taxonomy[contig]['best'] = __get_genus_from_tax_list(data[7].split(';'))
                taxonomy[contig]['best len'] = int(re.sub(",","",data[5]))
                taxonomy[contig]['common'] = data[8]
                taxonomy[contig]['tags'] = data[9]
                
        f.close()
    except IOError as (errno,strerror):
        print "I/O Error({0}): {1}".format(errno,strerror)
        return -1
    return seqs,taxonomy, most_common_org
...
def __get_blast_strings(id, tax_dict, length, first_key, second_key):
    if id in tax_dict:
        return tax_dict[id][first_key],"%.2f" % (float(tax_dict[id][second_key])/length * 100)
    return "NA","NA"
    
def __print_contig_stats(si_dict = None,cov_dict = None,tax_dict = None, most_common_org = None):
    #Contig scaffold, length, gc, cov (frg/jump) blast_hit blast_cvg ambig
    title = "Detailed Contig Stats"
    headers = ["Contig","Scaffold","Length","GC","Coverage(F/J/LR)","BLAST Hit","BLAST Covered", "Best BLAST Score", "Best Covered"]
    if most_common_org:
        headers += ["Most Common (" + most_common_org + ")", "SequenceAnnotations"]
    else:
    ...
    for c in sorted(si_dict.iterkeys()):
        ...
        name, len = __get_blast_strings(c, tax_dict, si_dict[c]['length'], 'genus', 'hit len')
        tmp += [name, len] 
        name, len = __get_blast_strings(c, tax_dict, si_dict[c]['length'], 'best', 'best len')
        tmp += [name, len]
...
def main():
    ...
    if options.tax_file:
        seq_info_dict,taxonomy_dict, most_common_org = __get_taxonomy(seq_info_dict, options.tax_file)

    __print_contig_stats(si_dict = seq_info_dict, cov_dict = bam_coverage_dict, tax_dict = taxonomy_dict, most_common_org = most_common_org)
