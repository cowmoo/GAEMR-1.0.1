...
parser.add_argument("--gc", "-g",type=str,help="please provide a reference file to calculate GC percentage", required=True)
...
parser.add_argument('--extended_name','-en', action='store_true', help='to help with instances where multiple BAMs have the same name but live in different dirs (uses part of path)')
...
def get_file_name_out_of_path(name_with_path):
    ...
    #return fields[-1]
    #use part of the path to make name unique in cases where multiple bams have the same name
    if args.extended_name:
...
#take the first n part's of the file path into the name to make it further unique based on path
def __make_filename_with_partial_path(self,filenamewithpath,n):
    """take the first n part's of the file path into the name to make it further unique based on path"""
    dirname, filename = os.path.split(os.path.abspath(filenamewithpath))
    dirname2, filename2 = os.path.split(os.path.abspath(dirname2))
    return os.path.join(dirname,filename2+"_"+filename)
...
def main_alt():
    ...
    #plot_coefficient_of_variation(bam_coverage_values,bam_names,gc_data,scaffold_break_coords, scaffold_break_names,minor_xtick_coords,minor_xtick_labels,chart_tools)
...
#plot the coefficient of variation in windows across the genome in cvrg and gv
def plot_coefficient_of_variation(cvrg_data,bam_names,gc_data,scaffold_break_coords, scaffold_break_names,minor_xtick_coords,minor_xtick_labels,chart_tools):
    """plot the coefficient of variation in windows across the genome in cvrg and gv"""

    window_size=4
    cvrg_var_coeffs_by_bam=[]
    for single_bam_data in cvrg_data:
        single_bam_coeffs=[]
        i=0
        while i < len(single_bam_data):
            window=[]
            j=0
            while j<window_size:
                window.append(single_bam_data[i])
                j+=1
                i+=1
            coeff = SimpleStat(window).get_coefficient_of_variation()
            single_bam_coeffs.append(coeff)
        cvrg_var_coeffs_by_bam.append(single_bam_coeffs)
            
    print cvrg_var_coeffs_by_bam
    ...
    while i<len(gc_data):
        ...
        j=0
        while j < window_size:
            window.append(gc_data[i][0])
            i+=1
            j+=1
        coeff = SimpleStat(window).get_coefficient_of_variation()
        gc_coeffs.append(coeff)

    obj_lines = chart_tools.gen_line_plot_with_layout_managed_gc_plot(cvrg_var_coeffs_by_bam
                                                                      ,bam_names,
                                                                      "Window",
                                                                      ...
                                                                      'GC Coefficients',
                                                                      "GC",
                                                                      "Coverage and GC% Relationship",
                                                                      "coefficients.gc_cvg",
                                                                      scaffold_break_coords,
                                                                      scaffold_break_names,
                                                                      minor_xtick_coords,
                                                                      minor_xtick_labels,True)
    obj_lines.save_as(args.output + ".coefficients.gc_cvg.png")
