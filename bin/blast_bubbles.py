...
def main():
    ...
    for line in open(args[0], 'r'):
        if line.startswith("#"):
            continue
        tmp = line.rstrip().split(options.delimiter)
        unit,group,length,gc,cov_str,org,id = tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],tmp[6]
        unitdata[unit] = []
    ...
    for line in open(args[1], 'r'):
        ...
        tmp = line.rstrip().split(options.delimiter)
        ctg,ctglen,hitlen,percov,tax = tmp[0],tmp[1],tmp[2],tmp[3],tmp[4]
        ...
        if (ctg != "#QueryId"):
            ...
            if (options.taxlevel in thisdata.keys()):
                ...
                try:
                    ...
                    unitdata[ctg].append(thisdata["common_name"])
                    organisms.add(thisdata["common_name"])
