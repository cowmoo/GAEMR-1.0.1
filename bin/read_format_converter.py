...
parser.add_option('--sample', '-s', action="store", default='sample', type='string', dest="sample",
                  help='The sample name.  Used for picard.jar FastqToSam software (default=%default)')
...
parser.add_option('--stringency', action="store", type='string', default='LENIENT', dest="stringency",
                  help='VALIDATION_STRINGENCY for Picard modules:  examples SILENT, LENIENT, STRICT, etc (default=%default)')
...
def __is_fastq_gz(extension, filelist):
    return extension == 'GZ' and __has_files(filelist)
...
def __check_files(files):
    ...
    for i in sorted(files.iterkeys()):
        ...
        if __is_bam(i,files[i]):
            ...
            file_object = BamFile(files[i][0], i, options.output, pairing, options.sample)
            num_files = 1
            break
...
def main():
    ...
    assert options.stringency in ('STRICT','LENIENT','SILENT')
    ...
    if check:
        ...
        ret_value = file_object.convert_to_unmapped_bam(java_heap, options.stringency)
        if ret_value:
