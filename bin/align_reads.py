...
parser = \
    OptionParser(usage='usage: %prog [options] -i unmapped.bam -r reference.fasta'
                 )
...
parser.add_option(
    ...
    default=True,
    dest='uses_mem',
    help='Align using bwa mem [default]',
)
...
if not options.output_header:
    options.output_header = options.input.rstrip('.bam'
                                                 ).rstrip('.unmapped') + '.aligned'
...
def main():
    ...
    if aligner == 'BWA' or aligner == "BWA_MEM":
        print 'output header:' + options.output_header
        bwa = BwaAlignment(
            options.input,
            options.reference,
            ...
            forced=options.unpaired,
            use_mem=options.uses_mem,
        )
        bwa.run_alignment(no_index=options.no_index, tmp_dir=options.tmp_dir,
                          mark_dups=options.mark_dups)
        bwa.clean_up_files()
    elif aligner == 'BOWTIE':
        bowtie = BowTieAlignment(
            options.input,
            options.reference,
            options.output_header,
            paired,
