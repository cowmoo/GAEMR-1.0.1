...
parser = OptionParser(usage="usage: %prog [options] scaffolds.fasta OR contigs.fasta if using --agp")
...
def main():
    ...
    a = Assembly(fasta=args[0], name=name, assembler=assembler, minGapSize=options.minGap,
                 minConSize=options.minContig, minScaffSize=options.minScaffold, agp=options.agp,
