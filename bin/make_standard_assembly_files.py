...
parser.add_option('--min_gap_size', '-g', action="store", default=constant.MIN_GAP_SIZE, type='int', dest="minGap",
    help='Minimum length of stretch of Ns to be considered gap sequence (default=%default)')
...
parser.add_option('--min_output_gap_size', action="store",default=100, type='int', dest="min_output_gap_size",
    help='Minimum gap size to use for output of small gaps.  Currently matches NCBI standard (default=%default)')
...
def main():
    ...
    if options.contigs and options.agp:
        a = Assembly(fasta=options.contigs, name=None, assembler=None, minGapSize=options.minGap,
                     minConSize=options.minContig, minScaffSize=options.minScaffold,
                     agp=options.agp, min_output_gap_size=options.min_output_gap_size)
    else:
        a = Assembly(fasta=options.scaffolds, name=None, assembler=None, minGapSize=options.minGap,
                     minConSize=options.minContig, minScaffSize=options.minScaffold,
                     agp=None, min_output_gap_size=options.min_output_gap_size)
