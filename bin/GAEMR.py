...
parser.add_option('--no_blast_filter',action="store_true",default=False,dest='no_blast_filter',
    help='Do not filter BLAST hits for redundancy (default=%default)')
parser.add_option('--no_rename', action="store_true", default=False, dest='no_rename',
    help='Do not rename scaffolds and contigs in fasta and agp files (default=%default)')
...
def run_blast_analysis(wu=None, query=None, threads=None, agp=None, blast_task=None, no_blast_filter=False):
    blast_xml=wu.run_blast(query=query, threads=threads, blast_task=blast_task)
    parsed_blast=wu.parse_blast_xml(blast_xml=blast_xml, no_blast_filter=no_blast_filter)
...
def __align_reads(wu=None, ar_dict=None, read_file_dict=None, db=None, db_header=None):
    ...
    for group in read_file_dict:
        for type in read_file_dict[group]:
            ...
            align_string = '-s'
	    # up length from 200 to a very high number to force bwa mem for paired reads
            if int(read_file_dict[group][type]['length']) > 2000000:
                align_string = '-l'
                #align_string = '-l'
                if type != 'unpaired':
                    aligner = 'BWA_MEM' #BowTie
...
def __run_analysis():
    ...
    scaffolds, contigs, agp = wu.standardize_file_inputs(scaffolds=options.scaffolds, contigs=options.contigs,
                                                         ...
                                                         minConSize=options.minConSize, minGapSize=options.minGapSize, rename=rename)
    ...
    if not options.no_blast:
        rb = Process(target=run_blast_analysis, args=(),kwargs={'wu':wu, 'query':contigs,'threads':options.threads, 'agp':agp,
                                                                'blast_task':options.blast_task, 'no_blast_filter':options.no_blast_filter})
        rb.start()
    ...
    if wu.check_pids():
        print "Some modules failed to run properly."
    else:
