...
class ChartUtilities:
    ...
    #crusher read GC uses this plot
    #generates a set 0f histograms,
    #data=dict of list of numbers. Key is name for legend and value is a list of numbers for the hist ( {'legend1':[], 'legend2':[] }
    #Please specify ouput
    #file name without any extensions.
    #if a "True" is given for ret_obj arg, a Chart object is returned versus a file
    #file being saved according to the name given. By default, a file is saved. Use the
    #file_name to specify where the file is to be saved. IE give full path to file name.
    def gen_multiple_histograms_in_same_plot_orig(self,all_data,x_name,y_name,main_name,output_file_name, ret_obj=False):
        """generate a simple bar plot of a list of numbers"""
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ...
        for legend_name,data in all_data.iteritems():
            n, bins, patches = ax.hist(data, 50, facecolor='#0099FF', alpha=0.75, histtype='step',label=legend_name)
            np.seterr(all='ignore')
            try:
                stats = SimpleStat(data)
                mu = stats.get_mean()
...
#--
    ...
    #file being saved according to the name given. By default, a file is saved. Use the
    #file_name to specify where the file is to be saved. IE give full path to file name.
    def gen_multiple_histograms_in_same_plot(self,all_data,x_name,y_name,main_name,output_file_name, ret_obj=False):
        """generate a simple bar plot of a list of numbers"""
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ...
        for legend_name,data in all_data.iteritems():
            ...
            try:
                ...
                bincs = 0.5*(binEdges[1:]+binEdges[:-1])
                y = mlab.normpdf( bincs, mu, sigma)
                l = ax.plot(bincs, y, '-', linewidth=1,label=legend_name)
            except:
                print "plotting histogram: note: unable to calculate standard deviation and mean due to data characteristics"
...
#--
    ...
    #4 analyze_vcf.py. plot_AB_to_position uses this
    ...
    def gen_multiple_scatters_on_same_plot(self,all_data,x_name,y_name,main_name,legend,output_file_name,chrom_break_positions,chrom_break_positions_labels,all_real_positions,ret_obj=False):
    ...
    #ACTIVE
    #analyze_read_gc.py of crusher uses this
    #composite chart function. Takes in a list of list of numbers. list[list]. Here each of inner lists
    #represent a single plot
    #if a "True" is given for ret_obj arg, a Chart object is returned versus a file
    #file being saved according to the name given. By default, a file is saved. Use the
    ...
    def gen_line_plot_with_two_y_scales(self,smaller_plot_values,
       ...
       # Put a legend below current axis
       ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.08),fancybox=True, shadow=True, ncol=1,prop=fontP)
       ...
       #commenting this out for now (3/7/2013) since it messes up the 2nd axis for crusher.analyze_read_gc.py.
       #other users "might" need this, so leaving it in for now and maybe adding a flag to activate it
       #but pretty sure this is buggy code that can be take out.
       #set x limit
    ...
    def gen_line_plot_with_layout_managed_gc_plot(self,smaller_plot_values,smaller_plot_labels,x_name,y1_name,large_plot_values,large_plot_label,y2_name,main_name,output_file_name,scaf_break_coords,scaf_break_labels,minor_xtick_coords, minor_xtick_labels,ret_obj=False):
       ...
       while index < len(smaller_plot_values):
           ...
           if color_index >= len(colors):
               color_index = 0
           plt.plot(single_plot_values,label=smaller_plot_labels[index],color=colors[color_index])
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,
    ...
    #primary user:read analysis tool (crusher) 
    #1. analyze_kmer_plot
    ...
    def gen_multiple_line_plots(self,all_chart_data,main,x_label,y_label,out_name,options=dict()):
        ...
        if options.has_key('xlim') and  options['xlim'] != 0:
            if options.has_key('xlim_start'):
                plt.xlim(options['xlim_start'],options['xlim'])
            else:
                plt.xlim(0,options['xlim'])
                #axe.xlim(0,xlim)
        ...
        if options.has_key('ylimit'):
            plt.ylim(0,options['ylimit'])
    ...
    def gen_multiple_scatter_plots(self,all_chart_data,main,x_label,y_label,out_name,options=dict()):
        ...
        axe.grid(True)
        if not options.has_key('no_ylim'):
            plt.ylim(0,40)
        if options.has_key('ylim'):
            plt.ylim(options['ylim'][0],options['ylim'][1])
        
        single_chart_data=values
        #set limits
        if options.has_key('xlim_both'):
            plt.xlim(options['xlim_both'][0],options['xlim_both'][1])
...
#all_data={'testdata1':data1, 'testdata2':data2}
#cu.gen_multiple_histograms_in_same_plot(all_data,'bin','freq','multi histogram test','multhist.test')
