...
class AgpFile:
    ...
    def __init__(self, agp_file=None, minGapSize=constant.MIN_GAP_SIZE, minConSize=1, minScaffSize=1, is_version_2=False):
    ...
    def print_agp_file_record_subset(self, scaffold_id, start=None, end=None):
        features = self.get_agp_file_record_subset(scaffold_id, start, end)
        for f in features:
            print f
    ...
    def get_agp_file_record_subset(self, scaffold_id, start=None, end=None):
        if self.__contains_scaffold(scaffold_id):
            ...
            for feature in range(start_feature, end_feature):
                features.append(self.__get_feature_record(scaffold_id,feature))
            return features
        else:
    ...
    # public function to get list of contig features based on scaffold id
    def get_contig_records_from_scaffold_id(self, scaffold_id):
        records = []
        for r in self.get_agp_file_record(scaffold_id):
            if not self.is_gap(scaffold_id, r):
                records.append(self.__get_feature_record(scaffold_id, r))
        return records

    # public function to get list of contig ids based on scaffold id
    def get_contig_ids_from_scaffold_id(self, scaffold_id):
        ids = []
        for r in self.get_contig_records_from_scaffold_id(scaffold_id):
            ids.append(r.split('\t')[5])
        return ids
    ...
    def get_contig_length(self, scaffold_id, contig_id):
        for f in self.get_agp_file_record(scaffold_id):
            if self.is_gap(scaffold_id, f):
                continue
            if self.get_contig_id(scaffold_id, f) == contig_id:
                return self.get_feature_length(scaffold_id, f)
