...
from libs.Bio import SeqIO
from libs.Bio.Seq import Seq
from libs.Bio.SeqRecord import SeqRecord
...
class FastqFile():
    ...
    def __init__(self, files, sample='sample', qual_format=None, direction=None, output=None):
        self.files = []
        self.sample = sample
    ...
    def __build_command(self, java_heap='32G', stringency='LENIENT'):
        ...
        validation_stringency = "VALIDATION_STRINGENCY=" + stringency
        
        num_files = self.__get_num_files()
        if num_files == 1:
            return ["java","-Xmx" + java_heap, "-jar",convert_command, "FastqToSam", "F1=",self.__get_single_file(0), "O=", self.get_output_file(),
                    sample_name,qual_arg,tmp_dir, validation_stringency]
        if num_files == 2:
            return ["java","-Xmx" + java_heap, "-jar",convert_command, "FastqToSam", "F1=",self.__get_single_file(0), "F2=",self.__get_single_file(1),"O=",
                    self.get_output_file(),sample_name,qual_arg,tmp_dir, validation_stringency]
        return 0
    ...
    def reverse_file(self, infile, outfile):
        cmd = "seqtk seq -r " + infile + " > " +  outfile
        subprocess.check_call(cmd, shell=True)
        
    def reverse_file_deprecated(self,infile,outfile):
        input = open(infile, 'r')
