...
class Taxonomy:
    ...
    def __init__(self, nodes_db=None, names_db=None, blast_db=None, gi_list=None):
        self.nodes_obj = Nodes(nodes_db)
        self.names_obj = Names(names_db)
        self.blastdbcmd_obj = BlastDbCmd(blast_db, gi_list)
        self.logger = logging.getLogger('gaemr.Taxonomy.Taxonomy')
    ...
    def get_accession_from_gi(self, gi):
        return self.blastdbcmd_obj.get_accession_from_gi(gi)

    def get_ordinal_id_from_gi(self, gi):
        return self.blastdbcmd_obj.get_ordinal_id_from_gi(gi)
    
    def get_sequence_title_from_gi(self, gi):
        return self.blastdbcmd_obj.get_sequence_title_from_gi(gi)
        
    def get_length_from_gi(self, gi):
        return self.blastdbcmd_obj.get_length_from_gi(gi)

    def get_tax_id_from_gi(self, gi):
        return self.blastdbcmd_obj.get_tax_id_from_gi(gi)

    def get_common_name_from_gi(self, gi):
        return self.blastdbcmd_obj.get_common_name_from_gi(gi)

    def get_scientific_name_from_gi(self, gi):
        return self.blastdbcmd_obj.get_scientific_name_from_gi(gi)
        ...
        if nodes and names:
            ...
            while nodes[tax_id]["parent"] and nodes[tax_id]["parent"] != tax_id and names[tax_id] != "root":
                if nodes[tax_id]["rank"] == 'no rank':
                    ...
                    if nodes[tax_id]["rank"] == "species":
                        tmp.append("tax_id=" + tax_id)
                    first = 0
        ...
        try:
            ...
            for lines in f.readlines():
                if re.search("scientific",lines):
                    ...
                    names_data[1] = re.sub("=","-",names_data[1])
...
class BlastDbCmd:
    ...
    def __get_info_helper(self, gi, key=None):
        if not self.__have_gi_info(gi):
            ...
            if not self.__have_gi_info(gi):
                self.blast_hit_info[gi] = {}
                self.blast_hit_info[gi][key] = "NOT_FOUND"
    ...
    def get_gi_tax_lookup(self, gi_list=None):
        ...
        for i in self.blast_hit_info.keys():
            if not self.blast_hit_info[i].has_key('tax id'):
                self.blast_hit_info[i]['tax id'] = "NOT_FOUND"
