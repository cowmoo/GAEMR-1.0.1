...
class Assembly(object):
    ...
    def __init__(self, fasta=None, name=None, assembler=None, minGapSize=constant.MIN_GAP_SIZE, minConSize=1, minScaffSize=1, agp=None, min_output_gap_size=constant.MIN_OUTPUT_GAP_SIZE):
        self.fasta = fasta

        self.minGapSize = minGapSize
        self.minConSize = minConSize
        self.minScaffSize = minScaffSize
        self.min_output_gap_size = min_output_gap_size
        ...
        if fasta:
            ...
            self.scaff_lookup_dict = self.__clean_scaffolds()
    ...
    def __read_fasta(self,fasta):
        ...
        for scaffold in SeqIO.parse(open(fasta, 'r'), "fasta"):
            ...
            if slen < self.minScaffSize:
                continue
            self.scaffolds.append(Scaffold(scaffold,self.minGapSize,
                                           self.minConSize, contig_ids=None,
                                           min_output_gap_size=self.min_output_gap_size))
    ...
    def __build_scaffold(self,agp,scaffold_id,f_dict,f_index):
        ...
        for i in f_dict:
            if agp.is_gap(scaffold_id,i):
                ...
                if length < self.min_output_gap_size:
                    length = self.min_output_gap_size
        ...
        if len(scaffold_seq) > self.minScaffSize:
            self.scaffolds.append(Scaffold(scaff_record,self.minGapSize,
                                           self.minConSize, contig_ids=contig_ids,
                                           min_output_gap_size=self.min_output_gap_size))
    ...
    def __to_dict(self,scaffolds):
        ...
        for i in scaffolds:
            ...
            scaffold_dict[i.get_name()] = i.get_output_seq()
    ...
    def get_scaffold_by_name(self,scaffold_id):
        ...
        if self.scaff_lookup_dict.has_key(scaffold_id):
            return self.scaffolds[self.scaff_lookup_dict[scaffold_id]]
        return 0
