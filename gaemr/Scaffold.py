...
class Scaffold:
    ...
    def __init__(self, scaffold, minGapSize, minConSize, contig_ids=None, min_output_gap_size=constant.MIN_OUTPUT_GAP_SIZE):
    ...
    def get_gaps_lengths_list(self):
        ...
        for f in self.contigs:
            if end == -1:
                ...
                gap_size = f.get_start() - end
                if gap_size < self.min_output_gap_size:
                    gap_size = self.min_output_gap_size
    ...
    def get_length(self):
        ...
        return self.get_gap_length() + self.get_contig_length()
    ...
    def get_output_seq(self):
        contigs, contigs_list = self.get_contig_sequences()
        gaps = self.get_gaps_lengths_list()
        
        seq = ""
        for i, c_name in enumerate(contigs_list):
            seq += contigs[c_name]
    ...
    #Calculate portion of scaffold to keep and then trim
        ...
        trimmed_seq = self.get_original_seq()[start:end]
